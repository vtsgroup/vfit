...
- [ ] **T17.1** — Auditar showcase.tsx (2238 linhas): listar todos componentes, marcar 12 não-usados em pages reais
  - Exemplos: `MD3CardHeader`, `MD3Tabs`, `ToolCard`, `NotificationCard`, `ActionCard3D`
  > Critério: CSV com nome | linhas | usados em | ação

- [ ] **T17.2** — Consolidar: `MD3Card` + `GlassCard` → única `Card` com variants
  > Remover 40 linhas de código duplicado

- [ ] **T17.3** — Dark mode contrast audit: testar todas as combinações de text/bg em light + dark
  > WCAG AAA: 7:1 for normal text, 4.5:1 for large (18pt+)
  > Ferramentas: Contrast Ratio Tool, Browser DevTools
  > Esperado: 0 violações

- [ ] **T17.4** — Corrigir violações identifikadas em T17.3 (ajustar CSS vars em dark mode)
  > Exemplo: `text-secondary` dark mode hoje ~3.2:1, aumentar para 4.5:1

- [ ] **T17.5** — Remover 12 componentes não-usados do showcase (não deletar global, só não exportar)
...
- [ ] **T17.8** — Type check + eslint → 0 erros
...
- [ ] **T19a.1** — Criar `StreakCalendar` component
...
- [ ] **T19a.2** — Endpoint `GET /api/v1/workouts/streak-history` (últimos 56 dias)
  > Retorna: `{ date, completed, km, minutes, xp }`

- [ ] **T19a.3** — Página `src/app/(app)/progresso/streaks/page.tsx`
  > Mostra calendar + current streak + best streak + próximo milestone

- [ ] **T19a.4** — Integrar em dashboard B2C (card "Sua Sequência: 12 dias 🔥")
...
- [ ] **T19b.1** — Redesign badges: hoje usam emojis (`🔥 🏆 💪`), converter para SVG icons + glow
  > Exemplo: Streak badge → fire icon em verde com glow

- [ ] **T19b.2** — Página `src/app/(app)/progresso/achievements/page.tsx`
...
- [ ] **T19b.3** — Endpoint `GET /api/v1/badges` (listar todos) + `GET /api/v1/users/me/badges` (earned)
  > JOIN earned badges com metadata

- [ ] **T19b.4** — Animação unlock: confetti + scale-up + glow (200ms spring)
  > OnClick em locked badge → tooltip mostra "Unlock: complete 50 workouts"
...
- [ ] **T19c.1** — Add "Share Plan" button no plano detail
  > Click → gera link referral `/referral/<code>` com plano preview
  > Share button (WhatsApp, Instagram, Twitter)
...
- [ ] **T20a.1** — Add date range picker em dashboard B2B
  > Presets: "Esta semana", "Este mês", "Últimos 90 dias", "Custom"
  > Update all charts dinamicamente

- [ ] **T20a.2** — Add student filter dropdown: "Todos" / "Ativos" / "Inativos" / "Por pagar"
  > Filter charts + stats cards

- [ ] **T20a.3** — New stat: "Ticket médio" = total_revenue / payment_count
  > Card: `€142.50 / transação` (visual)

- [ ] **T20a.4** — Breakdown chart: Revenue by plan tier (pie chart)
  > Trial vs Pro vs Profissional vs Max

- [ ] **T20a.5** — Trend analysis: month-on-month growth rate
  > Card: "↑ 12.5% versus mês passado"
...
- [ ] **T20b.1** — Create `src/app/dashboard/pipeline/page.tsx`
...
- [ ] **T20b.3** — Kanban-style board (ou vertical stacked chart)
  > Drag-drop cards between stages (future: automation)
  > Cada card: "João (5 workouts, R$0, -2 dias inativo)"

- [ ] **T20b.4** — Lead scoring: auto-assign status via heuristics
  > Prospect: signup < 7 dias atrás
  > Engaged: 1-5 workouts + last_activity < 3 dias
  > Retained: payment_status = 'paid'

- [ ] **T20b.5** — Quick actions: "Enviar mensagem", "Oferecer upgrade", "Verificar saúde"
  > Links para WhatsApp, email, payment status
...
- [ ] **T21b.1** — Find all `as any` (6 instâncias)
  > Enum: 4 are justified (PWA/iOS quirks), fix 2 others

- [ ] **T21b.2** — Type missing hooks: `use-dashboard.ts`, `use-vfit-nutrition.ts`, `use-plans.ts`
  > Add explicit return types

- [ ] **T21b.3** — Component props: `Button`, `Card`, `DSIcon` already typed? Verify.
  > Fix any union type issues

- [ ] **T21b.4** — tsc --noEmit → 0 errors
...
- [ ] **T21c.1** — Setup Playwright project (if not exists)
  > Exemplo: `npx playwright init`

- [ ] **T21c.2** — Create test files:
  - `tests/e2e/auth.spec.ts` (login, register, logout)
  - `tests/e2e/onboarding.spec.ts` (full B2C onboarding flow)
  - `tests/e2e/workout.spec.ts` (create, execute, complete)
  - `tests/e2e/checkout.spec.ts` (upgrade flow)

- [ ] **T21c.3** — Tests cover:
  > 1. User auth (login/register)
  > 2. B2C onboarding (17 steps)
  > 3. Plan generation (IA)
  > 4. Workout execution (complete + confetti)
  > 5. Payment (Asaas mock)

- [ ] **T21c.4** — CI/CD: run Playwright tests on every PR
  > GitHub Actions: `npm run test:e2e`
...
**Última revisão:** 07/04/2026 16:42 UTC
